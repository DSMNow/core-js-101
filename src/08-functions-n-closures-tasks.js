...
function getPowerFunction(exponent) {
  return function exponentFunc(x) {
    return x ** exponent;
  };
}
...
function getPolynom(...args) {
  return function polynoming(value) {
    switch (args.length) {
      case 3: return args[0] * (value ** 2) + args[1] * value + args[2];
      case 2: return args[0] * value + args[1];
...
function partialUsingArguments(fn, ...args1) {
  return function applying(...args2) {
...
function getIdGeneratorFunction(startFrom) {
  let IdValue = startFrom - 1;
  return function valueGrowth() {
    IdValue += 1;
    return IdValue;
  };
}
