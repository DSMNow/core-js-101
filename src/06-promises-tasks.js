...
function chainPromises(array, action) {
  ...
  let holder;
  return array.reduce((promiseCol, promiseI, i) => promiseCol.then((value) => {
    if (!holder) holder = value;
    else holder = action(holder, value);
    if (i < array.length - 1) return promiseI;
    return new Promise((resolve) => resolve(holder));
